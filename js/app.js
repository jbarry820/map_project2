...
function getFlickrPhotoUrl(photoset_id, photo_id, callback) {
  ...
  $.ajax({
    ...
    success: function(resp) {
      ...
      console.log(resp.photoset.photo.photoid)
      console.log(resp.photoset.photo.length);
      //var index = resp.map(function (img) { return img.value; }).indexOf(photo_id);
      //console.log(index);
      var url = getFlickrImageUrl(p); // why redefine url
      callback(url);
    },
  ...
  /*function getFlickrPhoto() {
    ...
    $.ajax({
      ...
      type: 'get',
      success: function(resp) {
        //var p = resp.photoset.photo[5];
        //var p = resp.photoset.photo[indexOf(clickedApiary)];
        console.log("testing");
        console.log(resp.photoset);
...
{
  console.log(p);
  console.log("https://farm" + p.farm + ".staticflickr.com/" + p.server + "/" + p.id + "_" + p.secret + ".jpg");
  return "https://farm" + p.farm + ".staticflickr.com/" + p.server + "/" + p.id + "_" + p.secret + ".jpg";
  //return "https://farm" + p.farm + ".staticflickr.com/" + p.server + "/" + 32389792544 + "_" + p.secret + ".jpg";
}
